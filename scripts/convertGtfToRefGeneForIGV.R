...
start <- aggregate(gtfInput$start - 1, by = list(transcript_id = gtfInput$transcript_id),
  FUN = min)
...
# Get nb of exons
nbExons <- aggregate(gtfInput$exon_id, by = list(transcript_id = gtfInput$transcript_id),
  FUN = length)
# Add it to the transcript data frame
transcripts$nbExons <- nbExons[match(transcripts$transcript_id, nbExons$transcript_id),
  "x"]
# Get the position of exon starts
startsExons <- aggregate(gtfInput$start - 1, by = list(transcript_id = gtfInput$transcript_id),
  FUN = paste, collapse = ",")
transcripts$startsExons <- startsExons[match(transcripts$transcript_id, startsExons$transcript_id),
  "x"]
# Get the position of exon ends
endsExons <- aggregate(gtfInput$end, by = list(transcript_id = gtfInput$transcript_id),
  FUN = paste, collapse = ",")
transcripts$endsExons <- endsExons[match(transcripts$transcript_id, endsExons$transcript_id),
  "x"]
...
if (nrow(gtfInput.cds) > 0) {
  ...
  leftCoding <- aggregate(gtfInput.cds$start, by = list(transcript_id = gtfInput.cds$transcript_id),
    FUN = min)
  transcripts$leftCoding <- leftCoding[match(transcripts$transcript_id, leftCoding$transcript_id),
    "x"]
  ...
  rightCoding <- aggregate(gtfInput.cds$end, by = list(transcript_id = gtfInput.cds$transcript_id),
    FUN = max)
  transcripts$rightCoding <- rightCoding[match(transcripts$transcript_id, leftCoding$transcript_id),
    "x"]
  ...
  transcripts$phaseExons[transcripts$transcript_id %in% gtfInput.cds$transcript_id] <- sapply(transcripts$nbExons[transcripts$transcript_id %in%
    gtfInput.cds$transcript_id], function(n) {
...
finalDataFrame <- data.frame(nb = 1:nrow(transcripts), id = transcripts$transcript_id,
  chr = transcripts$seqid, strand = transcripts$strand, start = transcripts$start,
  end = transcripts$end, scod = transcripts$leftCoding, ecod = transcripts$rightCoding,
  nbE = transcripts$nbExons, bE = transcripts$startsExons, eE = transcripts$endsExons,
...
write.table(finalDataFrame, paste0(dirname(gtfFile), "/RefGene.txt"), sep = "\t",
